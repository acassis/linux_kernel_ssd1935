...
// 2009.06.13 :
// we should includes this files in order to
// compile audio.ko module in kernel 2.6.24
// at ssd193x platform
// following include file is for 
// remap_pfn_range() call and some definitions like VM_WRITE ...
#include <linux/mm.h>
// following include file is for 
// ClearPageReserved(), ResetPageReserved() calls
...
// 2009.06.13 : 
// don't enable this flag to ssd193x platform with kernel 2.6.24
// because we cannot record fm dongle at sample rate higher than
// 16000 Hz
// FIXME :
// recording at sample rate higher than 16000 Hz if it is possible
//

// 2008.06.20 : 
// at str81xx platform with kernel 2.6.16 in order to recording
// at sample rate higher than 16000 Hz, we should enable 
// "LIMIT_ISO_MAXSIZE_PKT" definition
// limit isochrous transfter maxsize of packet
#define LIMIT_ISO_MAXSIZE_PKT
// TEST ONLY of following definition. NEVER open it at product fw
// 2008.06.19 : test for recording 44.1KHz and 48KHz of CM63xx FM dongle
///#define USBIN_FACTOR	1
//
...
#ifdef LIMIT_ISO_MAXSIZE_PKT
		unsigned int maxsize;    /* maximum size ... 2008.06.20 by SKY */
#endif
...
static int usbin_prepare_desc(struct usbin *u, struct urb *urb)
{
	...
	//printk(KERN_DEBUG "usbin_prepare_desc: maxsize %d freq 0x%x format 0x%x\n", u->maxsize, u->freqn, u->format);
	for (i = offs = 0; i < DESCFRAMES; i++, offs += u->maxsize) {
		urb->iso_frame_desc[i].length = u->maxsize;
#else
...
static int usbin_start(struct usb_audiodev *as)
{
	...
	int	temp, max = 0, is_out;
	
#ifdef LIMIT_ISO_MAXSIZE_PKT
	if (dev != NULL)
	{
#if 0
		if (dev->product) printk("%s(): product : %s\n", __FUNCTION__, dev->product);
		if (dev->manufacturer) printk("%s(): manufacturer : %s\n", __FUNCTION__, dev->manufacturer);
		if (dev->serial) printk("%s(): serial : %s\n", __FUNCTION__, dev->serial);
		printk("%s(): dev->speed : %d\n", __FUNCTION__, dev->speed);
		printk("%s(): wMaxPacketsize:%d\n", __FUNCTION__, dev->ep0.desc.wMaxPacketSize);
		printk("%s(): epnum : %d\n", __FUNCTION__, usb_pipeendpoint(u->datapipe));
#endif
		// from urb.c =====================================
		temp = usb_pipetype (u->datapipe);
		is_out = usb_pipeout (u->datapipe);
		max = usb_maxpacket(dev, u->datapipe, is_out);
...
#ifdef LIMIT_ISO_MAXSIZE_PKT
		// limit max size because of endpoint's max size of packet
		if (maxsze > max) maxsze = max;
		u->maxsize = maxsze;
#endif
	...
	// activate URB0 URB1 SYNC0 SYNC1
	u->flags |= FLG_RUNNING;
...
static int usb_audio_ioctl_mixdev(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	// 2008.07.02 : for identify the usb audio devices
	// we add "query vid/pid of usb device" by SOUND_MIXER_PRIVATE1
	if (cmd == SOUND_MIXER_PRIVATE1)
	{
		unsigned int val = 0;
		struct usb_device *usbdev = ms->state->usbdev;
		val = ((le16_to_cpu(usbdev->descriptor.idVendor) << 16) | \
	...
	if (cmd == SOUND_MIXER_PRIVATE2)
	{
		...
		int value = 0;
		struct usb_audiodev *as;
		list_for_each_entry(as, &ms->state->audiolist, list){
			value = (as->dev_audio - 3);
			while(value > 0)
			{
				val++;
				value -= 16;
...
// 2009.06.13 :
// at ssd193x platform with kernel version 2.6.24, we never
// found this problem because it will return error code
// with open option "O_NONBLOCK"
// 1. -ENODEV : if we unplug the dongle (FM CM63xx Tuner)
// 2. -EAGAIN : if recording data is not ready -> Resource temperarily unavaiable
// so we don't need to enable FIX_NEVER_BACK_AFTER_SCHEDULE_CALL
// because it will never be blocked at schedule() call
// we also can use "O_NONBLOCK" when we open this device for
// recording in nonblocking read mode
//

// 2009.06.12 :
// at str81xx platform with kernel version 2.6.16, we found
// usb_audio_read() may have a chance to be blockd in schedule() call
// and it will never go back (at least in my expirement).
// don't know why it is never going back from schedule() call
// maybe it is problem of kernel or str81xx platform
// try to enable FIX_NEVER_BACK_AFTER_SCHEDULE_CALL
///#define FIX_NEVER_BACK_AFTER_SCHEDULE_CALL
...
static ssize_t usb_audio_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
...
#ifdef FIX_NEVER_BACK_AFTER_SCHEDULE_CALL
	int maxtries = 5;
#endif
	...
	while (count > 0) {
		...
		if (maxtries < 0)
		{
			// let detector ( dongle unplug event )
			// close the fd
			///if (ret == 0) ret = -EAGAIN;
			break;
		}
		...
		if (cnt <= 0) {
			...
			// and it will never go back (at least in my expirement).
			// don't know why it is never going back from schedule() call
///			printk("%s(): goto schedule\n", __FUNCTION__);
			schedule();
///			printk("%s(): after schedule\n", __FUNCTION__);
			if (signal_pending(current)) {
				if (!ret)
					ret = -ERESTARTSYS;
				break;
