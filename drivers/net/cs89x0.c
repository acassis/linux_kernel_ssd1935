...
#include <asm/gpio.h>
static unsigned int netcard_portlist[] __initdata = {MAGUS_VIO_CS8900, 0};
static unsigned int cs8900_irq_map[] = {MAGUS_IRQ_CS8900, 0, 0, 0};
...
#if defined(CONFIG_MACH_IXDP2351) || defined(CONFIG_ARCH_MAGUS_ADS)
static u16
...
{
int	i; for (i = 0; i < ETH_ALEN; i++) dev->dev_addr[i] = (i * 2) + 2;
printk("Magus hardcoding MAC\n");
}
#endif
...
#if !defined (CONFIG_ARCH_MAGUS_ADS)
static void
...
//		set_irq_type(dev->irq, IRQ_TYPE_LEVEL_HIGH);
		ret = request_irq(dev->irq, &net_interrupt, 
				IRQF_DISABLED | IRQF_TRIGGER_HIGH, dev->name, dev);
#else
