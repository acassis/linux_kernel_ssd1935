/*
 * arch/arm/mach-magus/accio_p1.c
 */
...
static void accio_p1_power_off(void)
{
...
static void accio_p1_restart(char mode)
{
	...
	*(volatile uint32_t *)(watchdog_vadd | 0x00c) = 0x001;	// reset system after 1second.
	...
	mdelay(2000);
	printk("Reboot failed in accio_p1_restart().\n");
	while (1);
}
...
static void gpio_setting(void)
{
	...
	*(volatile uint32_t *)(MAGUS_VIO_GPIO | 0x318) = 0x00002225;
	*(volatile uint32_t *)(MAGUS_VIO_GPIO | 0x320) = 0x00002225;
	*(volatile uint32_t *)(MAGUS_VIO_GPIO | 0x33C) = 0x00000225;

	/*enable SDHC & TV-IN power off &LED enable*/
	*(volatile uint32_t *)(MAGUS_VIO_GPIO | 0x518) = 0x00C03000;
	*(volatile uint32_t *)(MAGUS_VIO_GPIO | 0x520) = 0x00C03000;
	*(volatile uint32_t *)(MAGUS_VIO_GPIO | 0x53C) = 0x00C02000;

	*(volatile uint32_t *)(MAGUS_VIO_GPIO | 0x318) |= 0x80000;
	*(volatile uint32_t *)(MAGUS_VIO_GPIO | 0x320) |= 0x80000;
	*(volatile uint32_t *)(MAGUS_VIO_GPIO | 0x33C) |= 0x80000;
#else
	/*disable CMMB module, codec 12M clk and power, disable amplifier*/
...
#endif
	pm_power_off = accio_p1_power_off;
	arm_pm_restart = accio_p1_restart;
	gpio_setting();
}
