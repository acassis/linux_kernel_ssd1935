...
static void accio_a2818t_power_off(void)
{
#if 0
	printk(KERN_DEBUG "Pull UP The Power Pin(PC11).\n");
	/* power down the board, PC11 */
	*(volatile uint32_t *)(MAGUS_VIO_GPIO | 0x218) |= 0x800;
	*(volatile uint32_t *)(MAGUS_VIO_GPIO | 0x220) |= 0x800;
	*(volatile uint32_t *)(MAGUS_VIO_GPIO | 0x23C) |= 0x800;
...
static void accio_a2818t_restart(char mode)
{
	...
	*(volatile uint32_t *)(watchdog_vadd | 0x00c) = 0x001;  // reset system after 1second.
	...
	mdelay(2000);
	printk("Reboot failed in accio_a2818t_restart().\n");
	while (1);
}
...
#endif
	pm_power_off = accio_a2818t_power_off;
	arm_pm_restart = accio_a2818t_restart;
	gpio_setting();
}
...
MACHINE_START(MAGUS_ACCIO, "Solomon Magus Accio A2818T")
	.phys_io		= MAGUS_IO,
