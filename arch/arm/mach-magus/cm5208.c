...
#ifdef CONFIG_ACCIO_CM5208
#include "lcdm_a030dn.h"
#endif
...
static void accio_cm5208_power_off(void)
{

	printk(KERN_ERR"Pull UP The Power Pin(PE4).\n");
	/* power down the cm5208 board, PE4 */
...
static void accio_cm5208_restart(char mode)
{
	...
	*(volatile uint32_t *)(watchdog_vadd | 0x00c) = 0x001;  // reset system after 1second.
	...
	mdelay(2000);
	printk("Reboot failed in accio_cm5208_restart().\n");
	while (1);
}
...
#endif
	pm_power_off = accio_cm5208_power_off;
	arm_pm_restart = accio_cm5208_restart;
	gpio_setting();
}
...
MACHINE_START(MAGUS_ACCIO, "Solomon Magus Accio CM5208")
	.phys_io		= MAGUS_IO,
