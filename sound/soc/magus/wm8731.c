...
extern int wm8731_thread(void *data);
	...
	unsigned int reg)
{
	...
	if (reg >= WM8731_CACHEREGNUM)
		return -1;
	//dbg("Codec REG_READ: 0x%xh = 0x%xh\n", reg*2, cache[reg]);
	return cache[reg];
}
	...
	unsigned int value)
{
	...
	data[1] = value & 0x00ff;
  dbg("Codec REG_WRITE: 0x%xh = 0x%xh\n", reg*2, value);
...
static int wm8731_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	struct snd_soc_codec *codec = socdev->codec;
	int i;
  dbg("enter wm8731_pcm_prepare func\n");
	
	/* set active */
	if(codec->restore_flag == 1) //deactivate then reactivate
        	wm8731_write(codec, WM8731_ACTIVE, 0x0000);
	wm8731_write(codec, WM8731_ACTIVE, 0x0001);

#if 0
	for(i=0;i <WM8731_CACHEREGNUM; i++)
	{
		dbg("wm8731 register 0x%xh=0x%xh\n", i*2, wm8731_read_reg_cache(codec,i));
  }
#endif  
	return 0;
}
...
static int wm8731_codec_restore(struct snd_soc_codec *codec, int flag)
{
  int i;
  if(flag)
  {
...
#endif
#if 1
#define WM8731_RATES (SNDRV_PCM_RATE_8000 |\		
		SNDRV_PCM_RATE_32000 | SNDRV_PCM_RATE_44100 |\
		SNDRV_PCM_RATE_48000 | SNDRV_PCM_RATE_88200 |\
		SNDRV_PCM_RATE_96000)
#endif
...
static int wm8731_init(struct snd_soc_device *socdev)
{
  ...
  dbg("enter wm8731_init func\n");
	codec->name = "WM8731";
	codec->owner = THIS_MODULE;
	codec->read = wm8731_read_reg_cache;
	codec->write = wm8731_write;
	codec->dapm_event = wm8731_dapm_event;
	codec->codec_restore = wm8731_codec_restore;
  ...
  /* to enable the DAC select by default */
  ...
  /* it will directly write WM8731 register */

  g_socdev = socdev;
  {
   wm8731_thread_t* thread_data = &wm8731_thread_data;
  dbg("enter wm8731 module init func\n");
  ...
  thread_data->thread_task = kthread_create(wm8731_thread, thread_data,"wm8731 thread");
	if (IS_ERR(thread_data->thread_task)) {
		//rc = PTR_ERR(thread_data->thread_task);
    printk("wm8731 thread create failed\n");
		goto card_err;
	}
  ...
  audio_ctl_cb_register(wm8731_audio_ctl_callback_func);
  //alsa_piu_enable(1);

  dbg("exit wm8731_init func Ok\n");
	return ret;
...
pcm_err:
  dbg("exit wm8731_init func Error\n");
...
/* Updated for Magus to attach WM8731 to SPI driver */

/*-------------------------------------------------------------------------*/
static int wm8731_codec_probe(struct spi_device *spi)
{
	struct snd_soc_device *socdev = wm8731_socdev;
	struct wm8731_setup_data *setup = socdev->codec_data;
	struct snd_soc_codec *codec = socdev->codec;
  ...
  dbg("enter wm8731_codec_probe func\n");
#if 0
  ...
  /* make the connection before invoke spi_write */
	...
	dbg("exit wm8731_codec_probe func Ok\n");
	return ret;

err:
	dbg("exit wm8731_codec_probe func Error\n");
	return ret;
}

static int __devexit wm8731_codec_remove(struct spi_device *spi)
{
  dbg("enter wm8731_codec_remove func\n");
...
static int wm8731_remove(struct platform_device *pdev)
{
 	...
 	if (thread_data->terminate == 0) 
  {
    ...
    dbg("WM8731 thread has been killed, exit module\n");
